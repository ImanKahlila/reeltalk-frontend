...
const CreateCommunityPage = () => {
  ...
  function descriptionInputChangeHandler(e: React.ChangeEvent<HTMLTextAreaElement>) {
    setDescriptionValue(e.currentTarget.value);
  }
  function rulesInputChangeHandler(e: React.ChangeEvent<HTMLTextAreaElement>) {
  ...
  const CreateCommunityPage = async () => {
    if (user) {
      try {
        ...
        const response = await axios.post('http://localhost:8080/communities', formData, {
          headers: {
  ...
  const [communityImagePreview, setCommunityImagePreview] = useState<string | null>();
        ...
        <div className='flex max-w-[120px] flex-col gap-4'>
          <picture className='relative block h-[153.787px] min-w-[120px] overflow-hidden rounded-md'>
            <Image
              ...
              src={communityImagePreview ? communityImagePreview : '/Pixel-160.png'}
              fill
              alt=''
            ></Image>
          </picture>
...
function RelatedMediaSelection({ id, poster, removeSelectionHandler }: IRelatedMediaSelection) {
  return (
    <div
